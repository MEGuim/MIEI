...
qsplit (x, y) = if (x > y) || (x==0 && y==0) then i1 () else i2 (media, ((x, media-1), (media+1, y)))
	where media = div (x+y) 2
\end{code}
...
Recebe um duplo de listas, retira o primeiro elemento da primeira lista para um par desse primeiro elemento e na segunda posição o duplo original sem a cabeçao da primeira lista. 
...
Na secção Sierp estão presentes funções para uma biblioteca, neste caso a TLTree. A função [geraSierp] gera um triângulo de Sierpinski através de um anamorfismo, em que o gene, a função [geneSierp] divide a estrutura do tipo Tri em três partes, para assim formar o triângulo, diminuindo o nível de recursividade em uma unidade.

Na função [tipsTLTree] recebe uma estrutura do tipo TLTree e trasforma-a numa lista de estruturas do tipo Tri.

A função [rep] transforma a estrutura TLTree em um triângulo de Sierpinski, que pode posteriormente ser visto numa página web. 
A função gera a estrutura do triângulo, chamando a função [geraSierp] e  posteriormente, deverá sofrer um [uncurryTLTree] que é o tipo de dados do input da função [tipsTLTree|, que vai transformar a TLTree numa lista de Tri's. 
Posteriormente cada elemento da lista vai ser passado para uma String própria para ficheiros html pela função [drawTriangle] e a função [finalize] junta tudo, para que seja possível ver o triangulo numa pagina html.
...
outTLTree (L a) = i1 a
outTLTree (N (a1, (a2, a3))) = i2 (a1, (a2, a3)) 
...
rep = finalize . concat . (map drawTriangle) . tipsTLTree . (uncurryTLTree geraSierp) . split fst snd
